...
void Camera::getPos( Vect &outPos) const
{
	outPos = this->vPos;
		...
		void Camera::getDir( Vect &dir) const
	{
		dir = this->vDir;
	}
	void Camera::getUp( Vect &upPos) const
	{
		upPos = this->vUp + this->vPos;
	}
...
void Camera::Orbit( void )
{
	Vect CamFocusPos;
	Vect focus;
	focus.set(0.0f,10.0f,10.0f) ;

	CamFocusPos = this->vPos - focus;

	float angle_y = 0.001f;
   	
	Matrix RotY( ROT_Z, angle_y );

	CamFocusPos *= RotY;

	CamFocusPos += focus;

	this->setOrientAndPosition( Vect(0.0f,0.0f,1.0f), Vect(0.0f,10.0f,10.0f), CamFocusPos );
}